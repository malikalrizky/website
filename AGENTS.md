...
- **src/data/career.json** — single source for CV text, work experience, education, certifications, skills (`npm run generate-cv`; `portfolio.js` maps company logos). Jobs and education entries use `startMonth` / `endMonth` (`YYYY-MM`); `type` (Full-time/Freelance), `engagement` (Permanent/Contract/Project), optional `companyTagline`, optional `regionalScope`, optional `group` (string ID — jobs with the same ID render as one company card on the site and one CV block via `src/utils/groupExperience.js`), optional CV-only fields `cvInclude` (false omits from PDF only), `cvMaxBullets`, `cvMaxProjects` (education), `cvOmitDesc`, and `promotionFrom`; PDF generator also caps certifications (`CV_MAX_CERTIFICATIONS`) and uses curated skills lines in `scripts/generate-cv.js`; display ranges via `src/utils/formatJobDates.js` (e.g. `Jun 2020 – Present`).
